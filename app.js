...
function showVersion() {
  ...
  badge.textContent = "version 0.1.21";
...
function initApp() {
  ...
  if (!toggleBtn) {
    ...
    document.getElementById("login-screen").appendChild(toggleBtn);
  }
  ...
  loginBtn.onclick = () => {
    ...
    if (isSignUp) {
      if (users[u]) {
        loginMsg.textContent = "User exists.";
        return;
      }
      users[u] = {
        password: p,
  ...
  function renderTeacher(t) {
    ...
    users[t].classrooms.forEach(code => {
      ...
      document.querySelector(`.custom-btn[data-code="${code}"]`).onclick = () => {
        // When opening, set dateInput only if blank
        if (!dateInput.value) {
          dateInput.value = new Date().toISOString().split("T")[0];
        }
        // Populate for that date
        const arr = cobj.customDrills[dateInput.value] || [];
        ta.value = arr.join("\n");
        editor.style.display = "block";
      };

      // Date change: repopulate textarea
      dateInput.onchange = () => {
        const arr = cobj.customDrills[dateInput.value] || [];
        ta.value = arr.join("\n");
      };
      ...
      document.querySelector(`.del-class[data-code="${code}"]`).onclick = () => {
        ...
        const up = getUsers();
        up[t].classrooms = up[t].classrooms.filter(c => c !== code);
        saveUsers(up);
        renderTeacher(t);
      };
  ...
  function renderStudent(code, student) {
    ...
    nextBtn.onclick = () => {
      ...
      const up = getUsers();
      up[student].progress[today] = up[student].progress[today]||[];
      up[student].progress[today].push({drill:idx,correct:corr,errors:errs,accuracy:acc});
      saveUsers(up);
  ...
  function enterAdmin() {
    ...
    Object.entries(users).forEach(([u,v]) => {
      let info = v.role==="teacher"
        ? (v.classrooms||[]).join(", ")
        : (v.classroomCode||"");
      const tr = document.createElement("tr");
      tr.innerHTML = `<td>${u}</td><td>${v.role}</td><td>${info}</td>
    ...
    document.querySelectorAll(".del-user").forEach(btn => {
      btn.onclick = () => {
        ...
        const up = getUsers(), cl = getClasses();
        if (up[u].role==="teacher") {
          up[u].classrooms.forEach(x=>delete cl[x]);
        } else {
          const cc = up[u].classroomCode;
          if (cl[cc]) cl[cc].students = cl[cc].students.filter(x=>x!==u);
        }
        delete up[u];
        saveUsers(up); saveClasses(cl);
        panel.remove(); enterAdmin();
      };
    });
  }
