...
window.addEventListener("DOMContentLoaded", () => {
  console.log("🔥 app.js v0.1.1 loaded");
  showVersion();
  initApp();
});
...
function showVersion() {
  ...
  badge.textContent = "version 0.1.1";
...
function initApp() {
  ...
  loginBtn.onclick = () => {
    ...
    if (isSignUp) {
      ...
      saveUsers(users); enterDashboard(name,role);
    } else {
  ...
  function enterDashboard(name,role){
    ...
    if (role==='teacher'){
      teacherNameEl.textContent = name; teacherDashboard.classList.remove("hidden"); classroomSetup.classList.remove("hidden"); teacherClassroomView.classList.remove("hidden"); renderTeacher(name);
    } else {
  ...
  createClassBtn.onclick=()=>{
    ...
    const classes=getClasses(); classes[code]={name:cname,teacher:teacherNameEl.textContent,students:[],drills:defaultDrills.slice(),customDrills:{}}; saveClasses(classes);
  ...
  function renderTeacher(teacher){
    ...
    document.querySelectorAll('.del').forEach(btn=>btn.onclick=()=>{
      ...
      const allClasses=getClasses(), students=allClasses[code].students||[];
      const usersMod=getUsers();
      students.forEach(s=>{ if(usersMod[s]) usersMod[s].classroomCode=undefined; });
      delete allClasses[code]; saveClasses(allClasses);
      usersMod[teacher].classrooms=usersMod[teacher].classrooms.filter(c=>c!==code); saveUsers(usersMod);
  ...
  function renderStudent(code,student){
    ...
    function updateAcc(){const spans=document.querySelectorAll('.char');const err=[...spans].filter(s=>s.classList.contains('error')).length;const total=spans.length;const acc=Math.max(0,Math.round((total-err)/total*100));accEl.textContent=`Accuracy: ${acc}%`;}    
    function load(){promptEl.innerHTML='';drills[idx].split('').forEach(ch=>{const span=document.createElement('span');span.className='char';span.textContent=ch;promptEl.appendChild(span);});pos=0;mark();feedbackEl.textContent='';nextBtn.disabled=true;accEl.textContent='Accuracy: 100%';}
    ...
    nextBtn.onclick=()=>{const spans=document.querySelectorAll('.char');const corr=[...spans].filter(s=>s.classList.contains('correct')).length;const err=[...spans].filter(s=>s.classList.contains('error')).length;const acc=Math.max(0,Math.round((corr/spans.length)*100));const users=getUsers();users[student].progress[today]=users[student].progress[today]||[];users[student].progress[today].push({drill:idx,correct:corr,errors:err,accuracy:acc});saveUsers(users);if(idx+1<drills.length){idx++;load();}else{promptEl.textContent='Done for today!';nextBtn.style.display='none';}};
    load();
  }
  ...
  function renderAdmin(){
    ...
    const adminDiv=document.getElementById('admin-dashboard'); adminDiv.innerHTML='<h2>Admin Panel</h2>';
    let html='<table border="1"><tr><th>User</th><th>Role</th><th>Action</th></tr>';
    Object.entries(users).forEach(([u,obj])=>{
      html+=`<tr><td>${u}</td><td>${obj.role}</td><td>`;
      if(obj.role==='student') html+=`<button data-user='${u}' class='del-user'>Delete Student</button>`;
      if(obj.role==='teacher') html+=`<button data-user='${u}' class='del-user'>Delete Teacher</button>`;
      html+='</td></tr>';
    }); html+='</table>';
    adminDiv.innerHTML+=html;
    adminDiv.querySelectorAll('.del-user').forEach(btn=>btn.onclick=()=>{
      const u=btn.dataset.user; if(!confirm(`Delete ${u}?`))return;
      ...
      if(allUsers[u].role==='teacher'){
        ...
        const cc=allUsers[u].classroomCode; if(cc && allClasses[cc]){
          allClasses[cc].students=allClasses[cc].students.filter(s=>s!==u);
        }
        delete allUsers[u];
      }
