...
function showVersion() {
  ...
  badge.textContent = 'version 0.2.32.Stable';
...
function initApp() {
 ...
 // ─── toggle sign-up/login ───
  ...
  function setupUserInLocalStorageFc(email, role, code) {
    ...
    if (!users[email]) {
      ...
      if (role==='student') {
        ...
        if (!cls[code]) { loginMsg.textContent='Class code invalid'; return false; }
        cls[code].students.push(email);
        saveClasses(cls);
      }
      saveUsers(users);
    }
...
loginBtn.onclick = async () => {
  ...
  // ─── create class ───
  createBtn.onclick = ()=>{
    const nm=newClassIn.value.trim(); if(!nm) return;
    const nc='C'+(100000+Math.floor(Math.random()*900000));
    const cls=getClasses(); cls[nc]={ name:nm, teacher:teacherName.textContent,
      students:[], drills:defaultDrills.slice(), customDrills:{} };
    saveClasses(cls);
    const us=getUsers(); us[teacherName.textContent].classrooms.push(nc);
