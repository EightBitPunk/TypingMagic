...
function initApp() {
  ...
  function renderTeacher(t) {
    ...
    (users[t].classrooms || []).forEach(code => {
      ...
      document.querySelector(`.del-class[data-code="${code}"]`).onclick = () => {
        ...
        const clsAll = getClasses(); delete clsAll[code]; saveClasses(clsAll);
      ...
      document.querySelectorAll(`.del-date[data-code="${code}"]`).forEach(btn => {
        btn.onclick = () => {
          ...
          const us2 = getUsers(), cl = getClasses();
          cl[code].students.forEach(s => { delete us2[s].progress[btn.dataset.date]; });
          saveUsers(us2);
          renderTeacher(t);
        };
      });
      ...
      fileIn.onchange = async e => {
        ...
        const resp = prompt('Apply to all classes?');
        if (!resp) return;
        ...
        lines.forEach(line => {
          ...
          if (applyAll) {
            users[t].classrooms.forEach(cid => { clsL[cid].customDrills = clsL[cid].customDrills||{}; clsL[cid].customDrills[date] = drills; });
          } else {
            clsL[code].customDrills = clsL[code].customDrills||{};
            clsL[code].customDrills[date] = drills;
          }
        });
        saveClasses(clsL);
        renderTeacher(t);
  ...
  function buildCalendar(student, code) {
    ...
    for(let i=0;i<first;i++){const tdEl=document.createElement('td');tdEl.style.padding='4px';tr.appendChild(tdEl);}    
    for(let d=1;d<=days;d++){if((first+d-1)%7===0&&d!==1){tbl.appendChild(tr);tr=document.createElement('tr');}
      const tdEl=document.createElement('td');tdEl.textContent=d;tdEl.style.width='24px';tdEl.style.height='24px';tdEl.style.textAlign='center';
      const key = `${year}-${String(m+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`;
      tdEl.style.cursor='pointer';
      if(d<todayD){tdEl.style.background=prog[key]?'lightgreen':'lightcoral';if(!prog[key])tdEl.onclick=()=>handlePast(code,key,student);else tdEl.onclick=()=>alert("Already done");}
      else if(d===todayD){tdEl.style.background=prog[key]?'lightgreen':'lightblue';}
      else {tdEl.style.background='lightgray';}
      tr.appendChild(tdEl);
    }
  ...
  function renderDrillsWithDate(code, drills, dateKey, student, isLate) {
    let idx=0,pos=0;const stats=document.getElementById('student-stats');stats.textContent='';
    function updateAcc(){const spans=document.querySelectorAll('.char');const errs=[...spans].filter(s=>s.classList.contains('error')).length;const pct=Math.round((spans.length-errs)/spans.length*100);stats.textContent=`Accuracy: ${pct}%`;}    
    function loadOne(){promptEl.innerHTML='';drills[idx].split('').forEach(ch=>{const sp=document.createElement('span');sp.className='char';sp.textContent=ch;promptEl.appendChild(sp);});pos=0;mark();feedbackEl.textContent='';nextBtn.disabled=true;nextBtn.textContent=idx<drills.length-1?'Next':'Submit';updateAcc();}
    function mark(){document.querySelectorAll('.char').forEach(s=>s.classList.remove('current'));document.querySelectorAll('.char')[pos]?.classList.add('current');}
    document.onkeydown=e=>{if(studentDash.classList.contains('hidden'))return;if(e.key==='Backspace'){e.preventDefault();if(pos>0){pos--;const ss=document.querySelectorAll('.char');ss[pos].classList.remove('correct','error');mark();updateAcc();nextBtn.disabled=true;}return;}if(e.key.length!==1||pos>=drills[idx].length){e.preventDefault();return;}const ss=document.querySelectorAll('.char');ss[pos].classList.remove('current');if(e.key===drills[idx][pos]){ss[pos].classList.add('correct');feedbackEl.textContent='';}else{ss[pos].classList.add('error');feedbackEl.textContent=`Expected "${drills[idx][pos]}" got "${e.key}"`;}pos++;mark();updateAcc();if(pos>=ss.length)nextBtn.disabled=false;};
    nextBtn.onclick=()=>{const ss=document.querySelectorAll('.char');const corr=[...ss].filter(s=>s.classList.contains('correct')).length;const errs=[...ss].filter(s=>s.classList.contains('error')).length;const pct=Math.round(corr/ss.length*100);const users=getUsers(),prog=users[student].progress;if(!prog[dateKey])prog[dateKey]=[];prog[dateKey].push({drill:idx,correct:corr,errors:errs,accuracy:pct,late:isLate});saveUsers(users);if(idx<drills.length-1){idx++;loadOne();}else{buildCalendar(student,code);promptEl.textContent='Typing Drill Completed!';nextBtn.disabled=true;}};loadOne();}
  function loadDrills(code,student){const cls=getClasses()[code];const today=new Date().toISOString().split('T')[0];renderDrillsWithDate(code,cls.customDrills[today]||cls.drills,today,student,false);}  
  
  // Admin panel
  function enterAdmin(){const ex=document.getElementById('admin');if(ex)ex.remove();const panel=document.createElement('div');panel.id='admin';panel.style.padding='1em';panel.innerHTML=`<h2>Admin Panel</h2><button id="cleanup-students">Delete orphan students</button><button id="cleanup-teachers">Delete orphan teachers</button><table border="1" style="width:100%;margin-top:1em;"><tr><th>User</th><th>Role</th><th>Info</th><th>Action</th></tr><tbody id="admin-body"></tbody></table>`;document.body.appendChild(panel);const users=getUsers(),classes=getClasses(),valid=new Set(Object.keys(classes)),body=document.getElementById('admin-body');body.innerHTML='';Object.entries(users).forEach(([u,d])=>{let info=d.role==='teacher'?(d.classrooms||[]).join(', '):(d.classroomCode||'');if(d.role==='student'&&!valid.has(d.classroomCode))info=`<span style="color:red">${info||'none'}</span>`;const tr=document.createElement('tr');tr.innerHTML=`<td>${u}</td><td>${d.role}</td><td>${info}</td><td><button data-user="${u}" class="del-user">Delete</button></td>`;body.appendChild(tr);});document.querySelectorAll('.del-user').forEach(b=>{b.onclick=()=>{const u=b.dataset.user;if(!confirm(`Delete ${u}?`))return;deleteUser(u);enterAdmin();}});document.getElementById('cleanup-students').onclick=()=>{
